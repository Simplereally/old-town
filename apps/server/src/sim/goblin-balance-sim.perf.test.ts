...
import { processCombatStartEvents, processDamageResolutionEvents } from "../systems/combat-system";
 ...
 * is already in melee range. This isolates the starter goblin's direct combat
 * balance from its separate proximity-aggression behavior.
 */
...
describe("mud_goblin balance regression", () => {
  ...
  it("mud_goblin is the level-5 aggressive North Quarry Road threat", () => {
    const def = registries.npc.get("mud_goblin");
    expect(def?.combatLevel).toBe(5);
    expect(def?.aggressiveRadius).toBe(3);
    expect(def?.aggressionMode).toBe("aggressive");
  });
});
