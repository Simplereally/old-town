/**
 * WebSocket transport protocol tests.
 *
 * These bind a real HTTP server + ws upgrade path. Filename
 * `*.integration.test.ts` places them in the heavy vitest lane (E50).
 */
...
describe("heavy: WebSocket transport", () => {
  afterEach(async () => {
    await cleanup?.();
    cleanup = undefined;
  });
