...
import { getMagicArmourFamilyFactory, getMagicArmourSlot, MAGIC_ARMOUR_FAMILIES } from "./index";

function getFactory(familyId: string): () => BufferGeometry {
  const factory = getMagicArmourFamilyFactory(familyId);
  if (!factory) throw new Error(`no factory for ${familyId}`);
  return factory;
}

describe("magic armour geometry factories (E41-S07)", () => {
  ...
  it("MAGIC_ARMOUR_FAMILIES has exactly 6 families", () => {
    expect(MAGIC_ARMOUR_FAMILIES).toHaveLength(6);
  });

  it("charmward maps to shield slot", () => {
    expect(getMagicArmourSlot("charmward")).toBe("shield");
  });
});
