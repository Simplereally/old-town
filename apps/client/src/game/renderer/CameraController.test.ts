...
const _created: CameraController[] = [];
...
function createController(): {
  ...
  _created.push(controller);
  return { camera, controls, controller };
}

afterEach(() => {
  for (const c of _created) c.dispose();
  _created.length = 0;
});
...
describe("CameraController", () => {
  ...
  it("does not hijack arrow keys while focus is in a text input", () => {
    const { controls, controller } = createController();
    controller.setIsometricAngle(false);
    const rotateLeft = vi.spyOn(controls, "rotateLeft");
    ...
    // Arrow key dispatched from the input bubbles to document, but the
    // controller must ignore it so caret movement stays intact.
