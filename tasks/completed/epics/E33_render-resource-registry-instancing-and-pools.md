...
- [X] Move completed story files into `tasks/completed/stories/E33/`.
...
- [X] Repeated object/prop/ground-item rendering uses bucket keys of `archetypeId + materialId + regionId + layer`.
- [X] Instance bucket writes are batched and flushed once per frame per bucket.
- [X] `mesh.count`, slot maps, free lists, dirty ranges, and bounds recomputation are handled deterministically.
- [X] Projectiles, hitsplats, click markers, hover markers, and decals use reusable pools or buckets with no per-frame allocations in hot update paths.
- [X] Actor visuals are render handles backed by pooled resources; gameplay/client store state does not hold Three objects.
- [X] Boundary checks fail if `apps/server`, `packages/shared`, or client net/store modules import Three or mutate `Object3D` transforms.
