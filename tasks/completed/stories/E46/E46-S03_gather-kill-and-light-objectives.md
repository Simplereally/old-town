...
- [X] Implement the inventory count trigger for `dry_log`.
- [X] Implement the kill count trigger for `cellar_rat`.
- [X] Implement the object interaction trigger for `market_kitchen_hearth` with `dry_log`.
- [X] Wire the quest trigger evaluation into the tick loop after the relevant phases.
- [X] Add quest var updates to the tick delta.
- [X] Write test: gathering 3 logs advances the quest stage.
- [X] Write test: killing 2 cellar rats advances the quest stage.
- [X] Write test: lighting the oven with logs advances the quest stage.

## Acceptance criteria

- [X] Gathering 3 dry logs advances the quest to the next stage.
- [X] Killing 2 cellar rats advances the quest to the next stage.
- [X] Lighting the oven advances the quest to the completion stage.
- [X] Triggers are evaluated server-side and cannot be skipped or faked by the client.
- [X] Quest var updates are included in tick deltas.
...
- [X] `bun run typecheck`
- [X] `bun run lint`
- [X] `bun run dev` — complete each quest objective
