...
- [X] **Verify combat flow:** Player selects "Attack" on a creature → server paths to melee range → combat starts → attacks every `attackSpeedTicks` → hit roll → damage → hitsplat delta → XP on hit → death at 0 HP → drop table roll → ground items with ownership → respawn timer.
- [X] **Verify creature aggression behavior:**
...
- [X] **Verify drop table rolls:** On creature death, the server rolls the drop table and spawns ground items. Verify:
...
- [X] **Verify respawn:** Creatures respawn at their home tile after `respawnTicks` (defined per NPC in `starter-npcs.json`).
- [X] **Verify client rendering:** The client must render health bars above creatures, hitsplat animations on hit, and ground items on death. Verify the client applies `hitsplats` deltas and `healthBar` entity updates.
- [X] **Verify combat XP distribution:** Combat XP is split to Attack/Strength/Defence/Hitpoints based on the player's active `combatStyle` (already implemented in `combat-system.ts`). The `pennywrought_shortblade` supports `stab`, `slash`, and `crush` styles. Verify XP is awarded correctly per style.
- [X] Write test: attacking a `cellar_rat` reduces its HP and generates hitsplat deltas (verify exists in combat tests).
- [X] Write test: killing a `cellar_rat` grants combat XP and drops `rat_drops` loot.
- [X] Write test: killing a `mud_goblin` drops `goblin_drops` loot.
- [X] Write test: a killed creature respawns after its `respawnTicks`.
...
- [X] Players can attack `cellar_rat` and `mud_goblin` via the "Attack" NPC option.
- [X] Combat uses attack/defence rolls (§13.4/§13.5) and shows hitsplats.
- [X] `cellar_rat` does not retaliate (peaceful); `mud_goblin` aggros within radius 3 (aggressive).
- [X] Creature death produces ground items from the correct drop table with ownership timing.
- [X] Respawn happens at the creature's home tile after `respawnTicks`.
- [X] Combat XP is distributed to the correct skill based on the player's active combat style.
- [X] Health bars and hitsplats are visible in the client.
...
- [X] `bun run dev` — kill a rat and a goblin
