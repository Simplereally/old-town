...
- [X] **Verify station placements:** Confirm all processing stations are placed in the map near their associated districts:
...
- [X] **Verify recipe routing:** Each station object must route to the correct recipe group. The `stationObjectIds` field in each recipe defines which stations accept it. Verify the intent dispatcher routes `smelt`/`cook`/`craft`/`tan`/`fire` actions to the correct station-recipe combinations.
...
- [X] **Verify beadwork loop:** `shape_bead_clay_blank` (lvl 1) converts `bead_clay` → `ember_bead` at `chalkhouse_bead_kiln`. The starter kit includes 20 each of `ember_bead`, `gust_bead`, `wit_bead`, `writ_bead` for testing the beadwork progression. Verify `bead_clay` is gatherable or purchasable so the loop is sustainable.
- [X] **Verify recipe selection UI:** The client recipe selection panel (E37-S01) must list available recipes for the clicked station, filtered by the player's current skill level. Verify the UI opens on station click and sends the recipe selection to the server.
- [X] Write test: smelting `penny_copper_ore` produces `pennywrought_ingot` and Smithing XP (verify exists in `skilling-system.test.ts`).
- [X] Write test: cooking `raw_fish` produces `cooked_fish` on success and `burnt_fish` on failure, with Cooking XP on success.
- [X] Write test: fletching `dry_log` produces `lathwood_arrows` and Bowcraft XP.
- [X] Write test: tanning `rabbit_hide` produces `soft_leather` and Tailoring XP.
...
- [X] Players can smelt `penny_copper_ore` into `pennywrought_ingot` at the foundry furnace.
- [X] Players can cook `raw_fish` into `cooked_fish` at the kitchen range; burns produce `burnt_fish`.
- [X] Players can fletch `dry_log` into `lathwood_arrows` at the bow bench.
- [X] Players can tan `rabbit_hide` into `soft_leather` at the tanning frame.
- [X] Players can shape `bead_clay` into `ember_bead` at the bead kiln.
- [X] Processing consumes inputs and produces outputs server-side.
- [X] Each processing action grants the correct skill XP.
- [X] Recipe selection UI is wired to the server and filters by skill level.
- [X] Stations enforce range and tool requirements.
...
- [X] `bun run dev` — run one recipe at each station
