...
- [X] **Verify level-1 node access:** The starter region (`old-town-0-0-0.json`) already has `dry_tree_node` (lvl 1), `scrub_tree` (lvl 1), and `copper_rock_node` (lvl 1) placed. Verify these are reachable from the Market Bell spawn (45, 45) without crossing impassable terrain.
- [X] **Add a level-1 tinstone node placement:** `tin_rock_node` (mining level 1, outputs `tinstone`) exists in `starter-nodes.json` but is not placed in any map. Add at least one `tin_rock_node` spawn in `old-town-0-0-0.json` near the quarry area (e.g., near the existing `copper_rock_node` at (30, 35)) so starters can mine tinstone without traveling to region 0-1-0. This supports the smithing loop in S03 which uses `penny_copper_ore` → `pennywrought_ingot`.
- [X] **Verify client rendering:** The client must render resource nodes in their normal and depleted (transformed) states. The server sends transform updates via entity update deltas. Verify the client applies the `transform` change to swap the object's visual model.
- [X] Write test: chopping `dry_tree_node` gives `dry_log` and Woodcutting XP (already exists in `skilling-system.test.ts` — verify it covers the starter nodes).
- [X] Write test: mining `copper_rock_node` gives `copper_ore` and Mining XP (already exists — verify coverage).
- [X] Write test: depleted nodes respawn after their `respawnTicks` (already exists in `resource-node-system.test.ts`).
...
- [X] Players can chop level-1 trees in the starter region for `dry_log` and Woodcutting XP.
- [X] Players can mine level-1 ore nodes for ore and Mining XP.
- [X] `tin_rock_node` is placed in the starter region so tinstone is accessible without region travel.
- [X] Nodes deplete and respawn on the 600 ms tick.
- [X] Tool requirements (axe for woodcutting, pickaxe for mining) are enforced server-side.
- [X] Inventory and XP deltas are broadcast to the client.
- [X] Visual state updates when a node depletes (transform applied on client).
...
- [X] `bun run dev` — chop a tree and mine an ore node
