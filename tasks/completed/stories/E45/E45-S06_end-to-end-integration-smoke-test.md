...
- [X] Write an integration test in `apps/server/src/sim/` that performs the following sequence via tick-driven commands (no client, no WebSocket — direct simulation kernel calls):
...
- [X] The test must use the real `SimulationKernel`, real content registries (loaded from `content/`), and the real region maps. No mocks of gameplay systems.
- [X] The test must run in under 30 seconds (tick-based, no real-time delays).
- [X] The test must be deterministic — use a seeded RNG so failure chances and drop rolls are reproducible.

## Acceptance criteria

- [X] The integration test spawns a character, gathers resources, processes them, fights a creature, banks loot, and gains XP across multiple skills.
- [X] The test passes with `bun run test`.
- [X] The test exercises the real simulation kernel, content registries, and region maps — no gameplay system mocks.
- [X] The test is deterministic (seeded RNG).
