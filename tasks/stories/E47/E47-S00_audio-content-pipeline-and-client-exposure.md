...
- [ ] Add `packages/shared/src/content-schemas/audio.ts` with schemas/types for audio definitions.
- [ ] Add `audio` to `packages/shared/src/content/content-kind.ts`.
- [ ] Add `content/audio` to the content directory-to-kind mapping used by the validator/loader.
- [ ] Export the audio schema from `packages/shared/src/content-schemas/index.ts`.
- [ ] Add `audio` to `ContentRegistries` and `contentClientRegistriesSchema`.
- [ ] Update `apps/server/src/content/serialize-content-for-client.ts` so audio definitions are included in `/api/content`.
- [ ] Add typed audio accessors to `apps/client/src/game/ui/ContentClient.ts`.
- [ ] Create minimal valid content files under `content/audio/` for ambience, positional sources, UI sounds, and action/server sounds, or one normalized `audio.json` if the schema chooses a single registry file.
- [ ] Add or document placeholder original audio asset paths under `assets/audio/`; do not add third-party/ripped game audio.
- [ ] Add tests proving `content/audio/*.json` validates and appears in the serialized client content payload.

## Acceptance criteria

- [ ] `bun run content:validate` accepts the new `content/audio/` files.
- [ ] Invalid audio content fails validation with clear messages.
- [ ] `/api/content` includes the audio registry.
- [ ] `ContentClient` can return audio definitions by ID/category.
- [ ] S01/S02 can add ambience and action/UI sounds without inventing another data path.
- [ ] All audio content is original, generated, or explicitly licensed for use.
