...
- [ ] Create `content/quests/smoke-over-old-town.json` with full stage/objective/trigger/reward definitions.
- [ ] Verify the quest schema supports inventory count triggers, kill count triggers, and object interaction triggers.
- [ ] Add the quest to the quest registry and content loader.
- [ ] Add validation that the quest's referenced items, NPCs, and objects exist.
- [ ] Write test: the quest JSON parses and all references resolve.
- [ ] Write test: the quest reward items exist in the item registry.
- [ ] Write test: the quest stages form a valid progression graph.

## Acceptance criteria

- [ ] The "Smoke Over Old Town" quest is defined in `content/quests/`.
- [ ] All quest stages, objectives, triggers, and rewards are content-driven.
- [ ] All referenced content IDs (items, NPCs, objects) exist.
- [ ] The quest schema validates the quest file.
- [ ] No hardcoded quest logic in the engine.
