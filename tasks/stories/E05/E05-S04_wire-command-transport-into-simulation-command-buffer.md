...
- [ ] Decode client command envelopes.
- [ ] Reject invalid commands with error packet or structured log.
- [ ] Route accepted commands to CommandBuffer.
- [ ] Protect against command spam with per-tick caps.
- [ ] Add tests for move command from socket through movement delta.

## Acceptance criteria

- [ ] Client movement occurs only after server tick processing.
- [ ] Duplicate command IDs are ignored.
- [ ] Out-of-range or malformed commands are rejected.
