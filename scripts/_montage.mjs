...
const cell = 124, cols = 7, pad = 6, rows = Math.ceil(names.length/cols);
const W = cols*(cell+pad)+pad, Hh = rows*(cell+pad+12)+pad;
const out = new PNG({width:W,height:Hh});
for (let i=0;i<out.data.length;i+=4){out.data[i]=24;out.data[i+1]=24;out.data[i+2]=26;out.data[i+3]=255;}
names.forEach((n,i)=>{
  const im = PNG.sync.read(readFileSync(resolve(DIR,n)));
  const cx=(i%cols)*(cell+pad)+pad, cy=Math.floor(i/cols)*(cell+pad+12)+pad;
  ...
  for(let y=0;y<dh;y++)for(let x=0;x<dw;x++){
    ...
    const si=(sy*im.width+sx)*4, di=((cy+y)*W+(cx+x))*4;
