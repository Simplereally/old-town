...
#### 1. `apps/client/src/game/ui/ContextMenu.ts` — Lines 95-114
...
#### 2. `apps/client/src/game/GameEngine.ts` — Lines 94-96
...
#### 3. `apps/client/src/game/ui/ContentClient.ts` — Line 33
...
#### 4. `apps/client/src/game/net/GameSocket.ts` — Lines 86, 112
...
#### 5. `apps/client/src/game/net/GameSocket.ts` — Lines 103, 114, 116, 119
...
#### 6. `apps/client/src/game/picking/EntityPicker.ts` — Line 44
...
#### 7. `apps/client/src/game/picking/EntityPicker.ts` — Lines 45-55
...
#### 8. `apps/client/src/game/GameEngine.ts` — Lines 428, 439
...
#### 9. `apps/client/src/game/ui/ContextMenu.ts` — Line 96
...
#### 10. `apps/client/src/game/picking/HoverHighlighter.ts` — Lines 62-66
...
#### 11. `apps/client/src/game/ui/UIState.ts` — Lines 124-128
...
#### 12. `apps/client/src/game/GameEngine.ts` — Line 711
...
#### 13. `apps/client/src/game/GameEngine.ts` — Line 848
...
#### 14. `apps/client/src/game/ui/UIManager.ts` — Lines 216, 225
...
#### 15. `apps/client/src/game/ui/UIManager.ts` — Lines 391, 397
...
#### 16. `apps/client/src/game/GameEngine.ts` — Lines 76, 278, 288, 300, 349, 353, 361, 369
...
#### 17. `apps/client/src/game/net/GameSocket.ts` — Lines 75, 80
...
#### 18. `apps/client/src/game/ui/UIManager.ts` — Lines 81, 84, 111, 112
...
#### 19. `apps/client/src/game/ui/ContextMenu.test.ts` — Lines 61, 73, 85
...
#### 20. `apps/client/src/game/ui/UIManager.test.ts` — Multiple lines (71, 72, 81, 89, 98, 106, 107)
...
#### 21. `apps/client/src/game/ui/UIState.ts` — Lines 15-18, 21
...
#### 22. `apps/client/src/game/ui/UIManager.ts` — Lines 38, 52
...
#### 23. `apps/client/src/game/ui/ContentClient.ts` — Lines 62-72
...
#### 24. `apps/client/src/game/ui/UIManager.ts` — Lines 291, 315, 341
...
#### 25. `apps/client/src/game/GameEngine.ts` — Lines 333-338 & `apps/client/src/game/ui/ContextMenu.ts` — Lines 221-226
...
#### 26. `apps/client/src/game/ui/UIManager.ts` — Line 226
...
#### 27. `apps/client/src/game/GameEngine.ts` — Line 123
**Anti-pattern:** Implicit coupling between `GameSocket.serverUrl` (WebSocket URL) and `ContentClient.load` (HTTP URL).
**Why:** `ContentClient` does string replacement (`ws://` → `http://`). If the server URL format changes, content loading breaks silently.
**Fix:** Have `GameSocket` expose an explicit `httpUrl` getter, or pass a separate HTTP URL to `ContentClient`.

---

#### 28. `apps/client/src/game/GameEngine.ts` — Line 77
...
#### 29. `apps/client/src/game/net/GameSocket.ts` — Line 86
...
#### 30. `apps/client/src/game/GameEngine.ts` — Lines 540, 590, 604, 609
...
#### 31. `apps/client/src/game/ui/UIManager.ts` — Line 53
...
#### 32. `apps/client/src/game/ui/ContextMenu.test.ts` — Line 115
...
#### 33. `apps/client/src/game/GameEngine.ts` — Line 518
...
### Duplicate Pattern 1: `EntityId` cast to `number`
...
### Duplicate Pattern 2: `plane` type assertions (`as TileCoord["plane"]`)
...
### Duplicate Pattern 3: `as` casts on Three.js `material` (array vs single)
...
### Duplicate Pattern 4: JSON/network data type assertions without validation
- **Lane 3 (protocol):** `decodeTransportMessage`, `decodeServerPacket`, `parseClientCommand`.
- **Lane 5 (client net/ui):** `GameSocket.ts`, `ContentClient.ts`.
- **Resolution:** Adopt a consistent policy: all JSON from the wire is `unknown` until validated with Zod. The project already uses Zod for content schemas; extend this to protocol and network boundaries.

### Duplicate Pattern 5: `as` casts in tests
