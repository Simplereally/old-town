---
doc_type: authority
canonical_path: docs/creatures/00-index.md
parent_index: docs/00-index.md
root_index: docs/00-index.md
---

Parent: [`Documentation Index`](../00-index.md)

Authority references:
- `POC_SPEC.md`
- `docs/world/00-index.md`
- `docs/skills/utility-skills.md`
- `docs/items/accessories/trophies.md`

# Creatures Index

> Creature ecology, combat roles, drop tables, Wardenry contracts, trophies, and minibosses.

## Documents

| Topic | Doc | Status | What is inside |
|-------|-----|--------|----------------|
| Creature System | [`creature-system.md`](creature-system.md) | Draft | Required creature fields and design rules |
| Starter Creatures | [`starter-creatures.md`](starter-creatures.md) | Draft | First Old Town creature roster |
| Creature Ecology | [`creature-ecology.md`](creature-ecology.md) | Draft | Creature placement by district |
| Combat Roles | [`combat-roles.md`](combat-roles.md) | Draft | Teaching roles for early enemies |
| Drop Table System | [`drop-table-system.md`](drop-table-system.md) | Draft | Drop table format and rules |
| Starter Drop Tables | [`starter-drop-tables.md`](starter-drop-tables.md) | Draft | Drops for first combat targets |
| Wardenry Contracts | [`wardenry-contracts.md`](wardenry-contracts.md) | Draft | Contract types and starter contracts |
| Rare Drops and Trophies | [`rare-drops-and-trophies.md`](rare-drops-and-trophies.md) | Draft | Trophy identity and rare reward rules |
| Minibosses | [`minibosses.md`](minibosses.md) | Draft | First named encounter targets |

## Core Rule

No creature should exist as a generic enemy with HP. Every creature needs ecology, combat role, drops, skill links, and reward identity.

